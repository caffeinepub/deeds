...
export default function FollowingModal({ userPrincipal, onClose }: FollowingModalProps) {
  const { data: following, isLoading } = useGetFollowing(userPrincipal);
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {following.map((follow) => (
                <FollowingItem key={follow.following.toString()} userPrincipal={follow.following} />
              ))}
            </div>
          ) : (
...
function FollowingItem({ userPrincipal }: { userPrincipal: Principal }) {
